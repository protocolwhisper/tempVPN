...
This Terraform creates one GCP VM intended to run the `vpn-node-daemon` WireGuard egress node from this TempVPN workspace.
...
Build a Linux daemon binary from the TempVPN workspace root:
...
```sh
gcloud compute scp ../../target/release/vpn-node-daemon \
...
In the TempVPN workspace root, create `vpn-client.toml` with an explicit registry URL:
