...
variable "node_id" {
  description = "Stable registry identifier for this exit node."
  type        = string
  default     = "us-central1"
}

variable "node_name" {
  description = "Display name published in the node catalog."
  type        = string
  default     = "US Central"
}

variable "registry_mode" {
  description = "Whether this daemon also serves the in-memory node registry."
  type        = bool
  default     = false
}

variable "registry_url" {
  description = "Stable public URL of the registry daemon. Empty disables self-registration."
  type        = string
  default     = ""
}

variable "registry_token" {
  description = "Dedicated registry-write token; it must differ from admin_token."
  type        = string
  sensitive   = true
}
