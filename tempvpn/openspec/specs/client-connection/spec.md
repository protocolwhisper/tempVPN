...
The client SHALL fail closed when a paid response is inconsistent with the
selected node or lacks fields needed to construct the tunnel.

#### Scenario: Paid response matches selection

- **GIVEN** a paid response from the selected node
- **WHEN** the client imports it
- **THEN** it activates the session against that same node
- **AND** accepts the activated response only when its node URL still matches
- **AND** requires an assigned tunnel address, server public key, and endpoint

#### Scenario: Activated response claims another node

- **WHEN** the activation response identifies a node other than the paid node
